...
until bash -c "kubectl get pods -n kube-system -l k8s-app=cilium | grep -q 'Init:CreateContainerError'" 
do
...
do 
    ...
    while [[ $(docker exec k3d-k3s-server-"${counts}" sh -c "mount --make-shared /run/cilium/cgroupv2") -eq 1 ]]
    do
...
do
    echo "working on docker exec for agent:" $counta
    while [[ $(docker exec k3d-k3s-agent-"${counta}" sh -c "mount --make-shared /run/cilium/cgroupv2") -eq 1 ]]
    do
        echo "Houston, we have a problem. Result is no good:"$?
        echo "trying mount again"
        sleep 5
